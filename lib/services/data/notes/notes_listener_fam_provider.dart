...
final notesListenerFamProvider = NotifierProvider.family<NotesListenerFamNotifier, List<NoteModel>?, String>(
  NotesListenerFamNotifier.new
);

class NotesListenerFamNotifier extends FamilyNotifier<List<NoteModel>?, String> {
  @override
  List<NoteModel>? build(String arg) {
    ...
    final subscription = db.watch(query).listen((results) {
      List<NoteModel> notes = results.map((e) => NoteModel.fromJson(e)).toList();
      state = notes;
    });
