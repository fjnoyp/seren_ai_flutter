...
final curUserAiChatThreadsListenerProvider = NotifierProvider<CurUserAiChatThreadsListenerNotifier, List<AiChatThreadModel>?>(
  CurUserAiChatThreadsListenerNotifier.new
);

/// Get the current user's chat threads
class CurUserAiChatThreadsListenerNotifier extends Notifier<List<AiChatThreadModel>?> {
  ...
  List<AiChatThreadModel>? build() {
    ...
    final subscription = db.watch(query).listen((results) {
      List<AiChatThreadModel> items = results.map((e) => AiChatThreadModel.fromJson(e)).toList();
      state = items; 
    });
